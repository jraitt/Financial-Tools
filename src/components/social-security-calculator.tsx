...
const formSchema = z.object({
  ...
  // Age 62 check - simplified to start of month
  const primaryMinDate = startOfMonth(addMonths(primaryBirth, 62 * 12));
  
  if (primaryClaim < primaryMinDate) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      message: "Cannot claim before age 62",
  ...
  if (data.maritalStatus === "Married") {
    ...
    const spouseMinDate = startOfMonth(addMonths(spouseBirth, 62 * 12));
...
export default function SocialSecurityCalculator() {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      try {
        const inputs = {
          maritalStatus: allValues.maritalStatus,
          primary: {
            birthDate: parseISO(allValues.primaryBirthDate),
            pia: allValues.primaryPia,
            claimDate: parseISO(allValues.primaryClaimDate + "-01"),
          },
          spouse: {
            birthDate: parseISO(allValues.spouseBirthDate),
            pia: allValues.spousePia,
            claimDate: parseISO(allValues.spouseClaimDate + "-01"),
          },
          spousalClaimDate: parseISO(allValues.spousalClaimDate + "-01"),
          inflationRate: allValues.inflationRate,
        };
  ...
  return (
    <div className="min-h-screen bg-muted/30 pb-20">
      {/* Hero Section */}
      <div className="bg-primary text-primary-foreground pt-8 pb-8 px-6 rounded-b-[2.5rem] shadow-xl relative overflow-hidden">
        <div className="absolute top-0 left-0 w-full h-full bg-[url('https://images.unsplash.com/photo-1579621970563-ebec7560ff3e?q=80&w=2071&auto=format&fit=crop')] opacity-10 mix-blend-overlay bg-cover bg-center"></div>
        <div className="max-w-7xl mx-auto relative z-10">
          <h1 className="text-3xl md:text-4xl font-bold tracking-tight font-sans leading-tight text-center">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-8 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <Card className="bg-card border-none shadow-md ring-1 ring-black/5 dark:ring-white/5">
                ...
                <CardContent className="p-6 flex flex-col justify-between h-full">
                  <div className="flex justify-between items-start mb-4">
                    <div className="p-2 bg-accent/10 rounded-full text-accent-foreground">
                     ...
                     <Badge variant="secondary" className="bg-accent/10 text-accent-foreground hover:bg-accent/20">Top Strategy</Badge>
                  </div>
                  <div>
